...
#include <stdint.h>
#include <time.h>
#include "include/mongoose.h"   // To build, run: cc main.c mongoose.c
...
void user_id_handler(struct mg_connection *c, struct mg_http_message *hm, uint32_t id) {
  if (id >= user_index) {
...
void user_handler(struct mg_connection *c, struct mg_http_message *hm) {
  if (mg_strcmp(hm->method, mg_str("POST")) == 0) {
    ...
    for (size_t i = 0; i < user_index; i++) {
    	offset += sprintf(response + offset, "user { id: %d, name: %s, reg_date: %ld }\n", users[i].id, users[i].name, users[i].reg_date);
    }
...
int main(void) {
  ...
  struct mg_connection* conn = mg_http_listen(&mgr, BIND, ev_handler, NULL);  // Setup listener
